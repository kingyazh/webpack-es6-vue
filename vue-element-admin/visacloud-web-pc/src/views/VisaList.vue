                ...
                <el-row :gutter="20">
                    <el-col :span="10" :offset="2">
                        <el-form-item label="国家" prop="country_code">
                            <el-select v-model="queryForm.country_code" placeholder="请选择" filterable clearable >
                                <el-option v-for="item in visaCountryOptions" :key="item.country_code" :label="item.country_cn_name" :value="item.country_code">
                                </el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
...
export default {
    ...
    computed:{
        showVisaType:function () {
            let country_code = this.queryForm.country_code;
            this.getVisaType(country_code);
            this.getVisaCenter(country_code);
            return !country_code?false:true;
        }
    },
    methods: {
        optOrderListFn() {
            let paramsObj = Object.assign({}, this.queryForm);
            this.$axios({
                method: 'POST',
        ...
        handleOperEdit(index, row) {
            this.$router.push({
                path: `/VisaOper/${row.opt_order_no}`
            });
        },
        handleInfoEdit(index, row,enterpriseId,visaOrderNo) {
            this.$router.push({
                path: `/VisaInfo/${enterpriseId}/${visaOrderNo}/${row.passport_no}`
            });
        },
            ...
            if (expandRows.indexOf(row) >= 0) {
                this.$axios.get(window.API_ROOT + 'rest/customer.list.query/v1', { params: { opt_order_no: row.opt_order_no } })
                    .then(resp => {
                        ...
                        !resp.data ? '' : resp.data.opt_order_no = row.opt_order_no,this.$set(row,"customerList",resp.data.list);
                    })
            }
        },
        getCountry() {
            return this.$axios.get(window.API_ROOT + 'rest/dict.query.country/v1',{params:{fetch_num:300}});
        },
        getDict(dict_type) {
        ...
        getVisaType(country_code) {
            ...
            return this.$axios.get(window.API_ROOT + 'rest/dict.query.visaType/v1',{ params: { country_code: country_code } })
                .then(visaTypeResp =>{
        ...
        getVisaCenter(country_code) {
            ...
            return this.$axios.get(window.API_ROOT + 'rest/dict.query.visaCenter/v1',{ params: { country_code: country_code } })
                .then(visaCenterResp =>{
        ...
        // 格式row的日期
        formatRowDate(row) {
            return UtilObj.formatUnixTimestamp(row.leave_date,'yyyy年MM月dd日');
        },
    },
    mounted() {
        this.optOrderListFn();
