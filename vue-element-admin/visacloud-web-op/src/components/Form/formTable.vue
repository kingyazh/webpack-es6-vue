        ...
        <el-main>
            <el-form :model="formData" :rules="formRule" ref="formData" size="mini" label-position="right" label-width="190px">
                ...
                <template v-for="(domain,key) in formElementsData.domains">
                    ...
                    <!-- // 可复制部分 -->
                    <!-- // 传 domainName 时，说明此部分需要进行复制，为表单的一个属性，且值为数组 -->
                    <div v-else-if="!!domain.name">
                        <h4 :id="domain.id" class="part_title">{{domain.title}} {{key}}</h4>
                        <el-row>
                            <template v-for="(input,rowIndex) in domain.rows">
                                ...
                                <span slot="label">
                                    ...
                                    <el-tooltip :content="!input.tool_tip?input.row_label:input.tool_tip" placement="left">
                                        ...
                                        <template v-for="(item, comIndex) in input.components">
                                            <formElement @updateformdata="updateArrayForm" :domainName="domain.name" :domainIndex="key" :key="comIndex" :elelabel="input.row_label" :ele="item" :elementValue="formElementsValue" :setHideEleKey="clearHideEleRule"/>
                                        </template>
                                    </el-form-item>
                                </el-col>
                            </template>
                ...
                <el-row type="flex" :gutter="20" align="center">
                    <el-col :span="12" :offset="6">
                        <template v-for="(domain,key) in formElementsData.domains">
                            <el-button v-if="!!domain.type" type="primary" @click="addNewItem(domain.name)">添加一条</el-button>
                        </template>
    ...
    export default {
        ...
        methods: {
            buttonEvent(button){
                ...
                let url = button.url?button.url.replace('/visacloud/',''):'';
            ...
            exportDataForm(url) {
                ...
                this.$nextTick(() => {
                    ...
                    this.$refs.formData.validate((valid,data) => {
                        ...
                        //         !res.data.url?this.$router.push('/'):window.open(res.data.url),this.$router.push('/');
                        //     }).catch((err) => {
                        //         console.log(err)
                        //     });
                        // } else {
            ...
            updateArrayForm(data){
                console.log(data);
                let dataArray = [],keyName = Object.keys(data)[0]
              !this.formData[keyName]?this.formData[keyName]={}:this.formData[keyName]
              let VisaFormEle = Object.assign(this.formData[keyName], data[keyName]);
              console.log(VisaFormEle);
              //  删除值为空的属性
                // for(let objKey in VisaFormEle){
                //     if(!VisaFormEle[objKey]){
                //         delete VisaFormEle[objKey];
