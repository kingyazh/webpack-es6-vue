          ...
          if (result) {
            AdminSet.changeName({
              ...
              if (resp.data['code'] === '10000') {
                this.$store.state.user.name = resp.data.user_info.name
        ...
        this.$refs['editMyPhone'].validate(result => {
          if (result) {
            AdminSet.updateUserPhone({
              ...
              if (resp.data.code === '10000') {
                if (resp.data.user_info) {
                  this.$store.state.user.phone = resp.data.user_info.phone
                } else {
                  this.$store.state.user.phone = this.editMyData.edit_phone
                }
                this.editMyDataDialog = false
              } else {
                // 根据返回的 code 判断是否修改成功，未修改成功，进行提示
