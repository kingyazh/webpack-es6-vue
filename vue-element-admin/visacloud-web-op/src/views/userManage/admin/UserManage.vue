                ...
                <el-table-column width="300" label="操作" align="center">
                    ...
                    <template slot-scope="scope">
                        <el-popover
                            ...
                            <el-form :model="editUserData" ref="editUserInfo" label-width="100px" :rules="editUserRules" status-icon>
                                <el-row :gutter="20">
                                    <el-col :span="22">
                                        <el-form-item label="姓名" prop="name">
  ...
  export default {
    data() {
      ...
      const phoneExistValidator = (rule, value, callback) => {
        if (value && this.editCurrentUser != -1 && this.userList[this.editCurrentUser].phone !== value) {
          checkPhoneExists({
            phone: value
          }).then(resp => {
            if (resp.status === 200) {
      ...
      const emailExistValidator = (rule, value, callback) => {
        if (value && this.editCurrentUser != -1 && this.userList[this.editCurrentUser].email !== value) {
          checkEmailExists({
            email: value
          }).then(resp => {
            if (resp.status === 200) {
      ...
      const newUserPhoneEmail = (rule, value, callback) => {
        if (this.addUserData.email || this.addUserData.phone) {
          callback();
        } else {
          callback(new Error('手机号码和邮箱不能全部为空'));
        }
      }
      const editUserPhoneEmail = (rule, value, callback) => {
        if (this.editUserData.email || this.editUserData.phone) {
          callback();
        } else {
          callback(new Error('手机号码和邮箱不能全部为空'));
        }
      ...
      return {
        ...
        addNewUserRules: {
          ...
          phone:[
            ...
            // { required: true, trigger: 'blur', validator: newUserPhoneEmail }
          ],
          email:[
        ...
        editUserRules: {
          ...
          phone:[
            { pattern: /^1[3456789]\d{9}$/, message: '目前只支持中国大陆的手机号码' },
            { required: true, trigger: 'blur', validator: phoneExistValidator },
            { trigger: 'blur', validator: editUserPhoneEmail }
          ],
          email:[
            { pattern: /^\w+\@+[0-9a-zA-Z]+\.(com|com.cn|edu|hk|cn|net|live|xin|cc|co|org|edu)$/, message: '填写正确的邮箱', trigger: 'blur' },
            { required: true, trigger: 'blur', validator: emailExistValidator },
