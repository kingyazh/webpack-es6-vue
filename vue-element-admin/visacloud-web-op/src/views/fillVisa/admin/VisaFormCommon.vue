...
import {getUserOrderInit,getUserOrderModel} from "@/api/FillVisa";
export default {
    ...
    created:function(){
        this.$axios
            .all([getUserOrderInit(this.FormParams),getUserOrderModel(this.FormParams)])
            .then(this.$axios.spread((UserVisaInitResp,UserVisaModelResp) => {
                console.log(UserVisaInitResp, UserVisaModelResp);
                // 维护操作单信息 表的简要内容
                this.$set(this.$data, "UserFormElements", !UserVisaModelResp.data ? [] : UserVisaModelResp.data);

                const getFormRules = window.UtilObj.makeRules(this.UserFormElements['domains']);
