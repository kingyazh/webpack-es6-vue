    ...
    <el-container>
        <el-main class="padding-small">
            ...
            <el-form :model="queryForm" ref="queryForm" label-width="130px" status-icon>
                <el-row :gutter="20">
                    <el-col :span="10" :offset="2">
                        <el-form-item label="团号" prop="country_code">
                            <el-input v-model="queryForm.team_no" placeholder=""></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="10">
                        <el-form-item label="客人姓名" prop="team_no">
                ...
                <el-row :gutter="20">
                    <el-col :span="10" :offset="2">
                        <el-form-item label="订单号" prop="visa_center_code">
                            <el-input v-model="queryForm.visa_order_no" placeholder=""></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="10">
                        <el-form-item label="护照号" prop="opt_status">
                ...
                <el-row :gutter="20">
                    <el-col v-if="currentTab==99" :span="10" :offset="2">
                        <el-form-item label="当前办理节点" prop="customer_name">
                ...
                <el-row :gutter="20">
                    <el-col :span="10" :offset="2">
                        <el-form-item v-if="currentTab==99" label="状态更新日期" prop="passport_no"><!--// 状态更新-->
                            <el-date-picker
                        ...
                        <el-form-item v-if="currentTab==2" label="收资料更新日期" prop="passport_no"><!--// 收资料操-->
                            <el-date-picker
                        ...
                        <el-form-item v-if="currentTab==3" label="审核资料日期" prop="passport_no"><!--// 审核资料-->
                            <el-date-picker
                        ...
                        <el-form-item v-if="currentTab==5" label="递交资料日期" prop="passport_no"><!--// 递交材料-->
                            <el-date-picker
                        ...
                        <el-form-item v-if="currentTab==6" label="出签更新日期" prop="passport_no"><!--// 出签管理-->
                            <el-date-picker
                    ...
                    <el-col :span="10">
                        <el-form-item v-if="currentTab==5" label="出签结果" prop="customer_name">
                ...
                <el-row v-if="currentTab==7" :gutter="20">
                    <el-col :span="10" :offset="2">
                        <el-form-item label="资料返还日期" prop="passport_no">
                            <el-date-picker
                    ...
                    <el-col :span="10">
                        <el-form-item label="资料返还类型" prop="customer_name">
                ...
                <el-row v-if="currentTab==7" :gutter="20">
                    <el-col :span="10" :offset="2">
                        <el-form-item label="资料返还操作日期" prop="passport_no">
                            <el-date-picker
            ...
            <el-row class="system_operate">
                <el-button type="primary" size="small" @click="multipleSelectOpen">批量维护办理进度</el-button>
            </el-row>
            <el-table :data="visaStatusList" border header-cell-class-name="table-head-th" :row-key="getRowKeys" ref="multipleTable" @selection-change="handleSelectUser">
                ...
                <el-table-column :key="Math.random()" v-if="currentTab==2" align="center" label="提交申请表时间" prop="init_date" :formatter="formatRowDate">
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab==2 || currentTab==3" align="center" label="收资料操作时间" prop="receive_date" :formatter="formatRowDate">
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab==2 || currentTab==3" align="center" label="收资料操作人" prop="receive_user">
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab==3 || currentTab==5" align="center" label="审核资料操作时间" prop="audit_date" :formatter="formatRowDate">
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab==3 || currentTab==5" align="center" label="审核资料操作人" prop="audit_user">
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab==5 || currentTab==6" align="center" label="递交资料操作时间" prop="submit_date" :formatter="formatRowDate">
                </el-table-column>
                ...
                <el-table-column :key="Math.random()" v-if="currentTab==6 || currentTab==7" align="center" label="上一节点操作时间" prop="record_result_date" :formatter="formatRowDate">
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab==6 || currentTab==7" align="center" label="上一办理节点操作人" prop="record_result_user">
                </el-table-column>
                ...
                <el-table-column :key="Math.random()" v-if="currentTab==7" align="center" label="资料返还操作时间" prop="return_insert_date" :formatter="formatRowDate">
                </el-table-column>
                ...
                <el-table-column :key="Math.random()" v-if="currentTab!=7" align="center" label="当前办理节点" prop="current_progress">
                    <!--// 点击编辑时的弹出框-->
                    <template slot-scope="scope">
                        <el-popover
                            ...
                            <el-table :data="scope.row.status_list" border header-cell-class-name="table-head-th">
                                ...
                                <el-table-column align="center" label="办理状态" prop="status">
                                    <template slot-scope="progress">
                                        {{customerStatusData[progress.row.status]}}
                                    </template>
                                </el-table-column>
                                <el-table-column align="center" label="操作人" prop="update_user">
                                </el-table-column>
                                <el-table-column align="center" label="操作时间" prop="update_date" :formatter="formatRowDate">
                                </el-table-column>
                                <el-table-column align="center" label="备注信息" prop="memo">
                                </el-table-column>
                            </el-table>
                            <span slot="reference" style="display:block;padding:10px 0;">{{currentProgressData[scope.row.current_progress]}}</span>
                        </el-popover>
                    </template>
                </el-table-column>
                <el-table-column :key="Math.random()" v-if="currentTab!=7" align="center" label="办理状态更新时间" prop="last_update_date" :formatter="formatRowDate">
                </el-table-column>
        ...
        <!--// 批量维护办理进度 弹出层操作界面-->
        <el-dialog title="" class="state-dialog" :visible.sync="userStateOperateDialog" @open="userStateDefaultSelection" @close="stepOne=true,stepTwo=0">
            <el-row slot="title">
                <i class="el-icon-question" @click="handleInfoDialog"></i>
            </el-row>
            <el-row v-if="stepOne" class="step-one">
                <h5 class="state-title"><i>01</i>选择办理进度</h5>
                <el-form :model="userStateOperateData" ref="userStateOperate" label-width="100px" status-icon>
                    <el-table :data="currentProgressOptions.filter(v => v.value != 1 && v.value != 4 && v.value != 99)" class="table-no-border" :border="false" :show-header="false" :row-key="getRowKeys" ref="userStateOperateTable" @select="handleSelectState">
                        <el-table-column align="center" :selectable='userStateSelectAble' type="selection" width="30">
                        </el-table-column>
                        <el-table-column label="办理进度" prop="label">
                            <template slot-scope="scope">
                                {{scope.row.label}}
                                <span v-if="scope.row.value == 2">（确认收到客人资料，可以开始办理签证）</span>
                    ...
                    <el-row>
                        <el-col class="dialog-btn">
                            <el-form-item>
                                <el-button type="primary" @click="submitForm('userStateOperate')">下一步</el-button>
            ...
            <el-row v-if="!stepOne" class="step-two">
                    ...
                    <el-form :model="statusUpdateData" ref="step_2" label-width="100px" status-icon>
                        <table class="info-show-table">
                            ...
                            <tr>
                                ...
                                <td class="img">
                                    <img src="http://file2.zhituad.com/thumb/201311/23/201311231053275427O477k_priv.jpg" alt="" @mouseover="imgHoverFn" @mouseout="imgOutFn">
                                    <img src="http://file2.zhituad.com/thumb/201311/23/201311231053275427O477k_priv.jpg" alt="" @mouseover="imgHoverFn" @mouseout="imgOutFn">
                                    <img src="http://file2.zhituad.com/thumb/201311/23/201311231053275427O477k_priv.jpg" alt="" @mouseover="imgHoverFn" @mouseout="imgOutFn">
                                </td>
                            </tr>
                        ...
                        <el-row class="dialog-btn">
                            <el-form-item>
                                ...
                                <el-button type="primary" @click="submitForm('step_2')">{{stepTwo==2?'收资料':'审核资料'}}</el-button>
                            ...
                            <el-col :span="22">
                                <el-form-item label="办理结果" prop="visa_result">
                                    <el-select v-model="statusUpdateData.submit_result" placeholder="请选择" clearable >
                                        <el-option v-for="item in visaResultOptions" :key="item.code" :label="item.value" :value="item.code">
                                        </el-option>
                                    </el-select>
                                </el-form-item>
                            </el-col>
                ...
                <div v-if="stepTwo == 7">
                    <el-form :model="statusUpdateData" ref="step_7" label-width="100px" status-icon>
                        <el-row class="step-form">
                            <el-col :span="22">
                                <el-form-item label="返回类型" prop="visa_result">
                            ...
                            <el-col>
                                <el-form-item label="资料返还日期" prop="visa_result">
                                    <el-date-picker v-model="statusUpdateData.express_date" format="yyyy 年 MM 月 dd 日" value-format="timestamp" class="width-all" type="date" placeholder="选择日期" :clearable="false">
                                    </el-date-picker>
                                </el-form-item>
                            </el-col>
                            <el-col>
                                <el-form-item label="快递公司" prop="visa_result">
                                    <el-input v-model="statusUpdateData.express_company" placeholder=""  style="width: 100%;"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col>
                                <el-form-item label="快递单号" prop="visa_result">
        ...
        <el-dialog title="办理进度说明" :visible.sync="StateInfoDialog" width="45%" @close="">
            <el-row>
                <h6>操作说明：</h6>
                <p>1、只可选择未维护的办理进度；</p>
    ...
    methods: {
      ...
      visaStatusOperateFn(formName){
        ...
        visaStatusUpdate(paramsArray).then((res) => {
          // 减少接口请求，是否可以本地更新数据
          this.visaStatusListFn(countryCode)
          // 清除表单选项
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            if(formName === 'userStateOperate'){
              ...
              // console.log(this.queryForm.current_progress);
              if(this.currentTab==99){
                this.queryForm.current_progress = ''
              }
      ...
      userStateOperateEdit(index,row){
        ...
        // 默认选中下一步，减少操作
        this.$set(this.$data,'stepTwo',row['current_progress']-0+1)
      ...
      handleTabClick(tabId){
        ...
        this.queryForm.current_progress = tabId
      ...
      handleSelectUser(user) {
        ...
        for(let i=0;i<user.length;i++){
          ...
          if(this.stepInit>user[i]['current_progress']){
    ...
    created(){
      ...
      this.$axios
        ...
        .then(this.$axios.spread((visaResultResp, returnTypeResp,currentProgressResp,customerStatusResp) => {
          ...
          // 客人状态
          this.customerStatusOptions = !customerStatusResp.data ? [] : customerStatusResp.data.map(item => {
