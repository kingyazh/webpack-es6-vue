...
<script>
    ...
    export default {
        ...
        methods: {
            //  get optFrom data
            getOptOrderInit() {
                return this.$axios.get(window.API_ROOT + 'rest/optOrder.update.init/v1', { params: this.FormParams });
            },
            //  get optFrom form elements
            getOptOrderModel() {
                return this.$axios.get(window.API_ROOT + 'rest/optOrder.update.model/v1', { params: this.FormParams });
            },
            //  made the form table
            // madeFormTable(){
            //
        ...
        watch:{
            formElements:function(){
                //  注释掉验证，先做表单提交
                // let formRules = Util.makeRules(this.formElements['domains']);
        ...
        mounted: function () {
            this.$axios
                .all([this.getOptOrderInit(),this.getOptOrderModel()])
                .then(this.$axios.spread((OptOrderInitResp,OptOrderModelResp) => {
                    // 维护操作单信息 表的简要内容
                    OptOrderInitResp.data.leave_date = Util.formatUnixTimestamp(OptOrderInitResp.data.leave_date,'yyyy-MM-dd');
